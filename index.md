...
Thus, the total number of possibilities is \\(O(5^{N^2})\\). (Note that this is a very loose upper bound.) Thus, checkers is in EXPTIME, as _many_ of these positions could be reached in at least one way by normal play, and an algorithm has to go down every one! [further topics: machine learning of games vs. “solving” of games]
